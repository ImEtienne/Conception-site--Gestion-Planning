...
class SeanceController extends Controller
{
    ...
    public function showList(Request $request, $id){
        //$cour = Cours::where('cours', $request->id)->first();
        //$cour = Cours::findOrFail($id);
        $cour= DB::table('cours')->where('id', $request->id)->first();
        $seance = DB::table('seances')->where('cours_id', $cour->id)->get();
        //$seance = Seances::where('cours_id', '=', $id)->get();
        return view('gestionnaire.Seances.index', ['seances'=>$seance]);
    }
    ...
    public function showListePresenceDetaille($id){
        ...
        $presence = DB::table('presences')->where('etudiant_id', $id)->get();
        return view('gestionnaire.Seances.presenceDetailleeParEtudiant', ['presences'=>$presence, 'etudiants'=>$etudiant]);
    }

    //Liste des présences (des étudiants) par séance.
    public function showListePresencesParSeance($id){
        $s = Seances::findOrFail($id);
        $q = DB::table('presences')->where('seance_id', $id)->get();
        return view('gestionnaire.Seances.presenceEtudiantParSeance', ['presences'=>$q, 'seances'=>$s]);
    }
